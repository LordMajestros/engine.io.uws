...
function Server(opts){
  ...
  this.maxHttpBufferSize = opts.maxHttpBufferSize || 2048;//10E7;
  ...
  if (~this.transports.indexOf('websocket')) {
    this.ws = new WebSocketServer.Server({
      bufferSize: this.maxHttpBufferSize, 
      maxMessageSize: this.maxHttpBufferSize,
      path: opts.path || false,
      server: opts.server || false,
      keepAliveTime : opts.pingTimeout,
      keepAliveInterval : opts.pingInterval,
      //keepAliveRetry : 7,//???needed
      perMessageDeflate: this.perMessageDeflate
      //ssl ???// handled by node http server (planned lws support)
    });
    this.ws.perMessageDeflate = this.perMessageDeflate;
    this.wsServer = new wsServer(this.ws);
  }
}
...
Server.prototype.handshake = function(transportName, req, nativeSocket){
  ...
  if(nativeSocket){
    if(transport.setSocket){
      transport.setSocket(nativeSocket);
    }
  }
...
Server.prototype.handleUpgrade = function(req, socket, upgradeHead){
  ...
  this.verify(req, true, function(err, success) {
    ...
    // delegate to lws
    var conn = self.ws.handleUpgrade(socket, req, upgradeHead);
    if(conn){
      debug('Native upgrade successful');
      self.onWebSocket(req,conn);
    }
    else{
        debug('Native upgrade failed');
    }
    upgradeHead = null;
...
Server.prototype.onWebSocket = function(req, socketHandle){
  socket = this.wrap(socketHandle);
  ...
  if (id) {
    ...
    this.handshake(req._query.transport, req,socket);
  }
...
Server.prototype.wrap = function (socketHandle) {
  var webSocket = new wsSocket(socketHandle,this.ws);
  this.ws.setUserData(socketHandle, webSocket);
  return webSocket;
};
...
function wsSocket(lwsSocket, server) {
  ...
  this.send = function (message, options,callback) {
    if (wsSocket.lwsSocket !== null) {
      ...
      if (!this.server.perMessageDeflate || this.server.perMessageDeflate.threshold>message.length) {
...
function wsServer(server) {
  ...
  server.unlink = function (wsSocket) {
    ...
    wsSocket.engineWebSocket = null;
  };

  server.on('message', function (socket, message, binary, bytesRemaining) {
    var ws = server.getUserData(socket);
    if(ws.engineWebSocket && ws.engineWebSocket.onData){
      ws.engineWebSocket.onData(''+message);
    }
    else{
      debug('No engineWebsocket on this socket');
    }
  });
...
util.inherits(wsSocket, require('events'));
util.inherits(wsServer, require('events'));
