...
Socket.prototype.onError = function (err) {
  ...
  if(err.description && err.description==1009){
      this.onClose('maxpayload error',err);
      return;
  }
...
Socket.prototype.sendPacket = function (type, data, options, callback) {
  ...
  if(data){
      ...
      if(this.transport.name=='websocket' && options.compress && this.server && this.server.perMessageDeflate && this.server.perMessageDeflate.threshold && this.server.perMessageDeflate.threshold > 0 && this.server.perMessageDeflate.threshold > len) {
...
Socket.prototype.closeTransport = function (discard) {
  if (discard) {
    ...
    /*if(this.upgradingTo) {
      debug('Discarding upgrading to');
      this.upgradingTo.discard();
    }*/
  }
  this.transport.close(this.onClose.bind(this, 'forced close'));
/*  if(this.upgradingTo) {
    debug('Closing upgrading transport');
    this.upgradingTo.close();
    this.upgradingTo.socket.close();
  }*/
};
