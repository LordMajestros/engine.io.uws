...
Server.prototype.handleUpgrade = function(req, socket, upgradeHead){
  ...
  this.verify(req, true, function(err, success) {
    ...
    // delegate to lws
    var conn = self.ws.handleUpgrade(req, socket, head,function(conn){
        if(conn){
...
Server.prototype.attach = function(server, options){
  ...
  if(~self.transports.indexOf('websocket')) {
    ...
    var uwsOpts = {
        //server:server,//don't pass a server so we can use our handleUpgrade
        path:path,
        bufferSize: this.maxHttpBufferSize, 
        maxMessageSize: this.maxHttpBufferSize
    };
    this.ws = new uWebSockets.Server(uwsOpts);
    //override native server onconnection to prevent emits
    this.ws.nativeServer.onconnection = function(){};
